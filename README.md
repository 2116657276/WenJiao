...
- 📜 **一张连续宣纸的东方复古风**：全窗为单一宣纸纸面（`#F3F0E6` 浅灰暖色），以极淡的 CSS/SVG 纸纹（feTurbulence 噪点 + 纸纤维细线）、极细接缝线、朱砂篆印与宋体墨字构建层级秩序；不再使用分块底色、卡片、玻璃模糊与高光。全站杜绝任何 Emoji。
- ⚖️ **严格的数学概率平衡（公平三圣）**：
...
本项目提供两套极轻量、无任何臃肿运行时的桌面客户端方案。**两套实现的窗口规则完全一致**：内容区固定 **560 × 780**（约占 Mac 屏幕四分之一稍多的竖向小窗）、正常 macOS 原生标题栏（红黄绿系统按钮、可自由拖动到桌面任意位置）、不可缩放、不可最大化、启动居中。

- **macOS 推荐构建方式：方式一（原生 Cocoa / WebKit 版）** —— 体积最小、内存最低、无任何额外运行时，日常自用首选。
- **Windows / 跨平台构建方式：方式二（Tauri v2 版）** —— 需要产出 Windows `.exe` 或跨平台分发时使用。

### 1. 超轻量 macOS 原生版 (~55 KB) 【macOS 推荐】
- 基于 Apple Clang 与 macOS 自带的 `Cocoa` / `WebKit.framework` 原生构建。
- 二进制体积仅约 **55 KB**，内存常驻仅 **~25 MB**。
- 窗口为固定尺寸的小型竖向桌面工具（560 × 780），`resizable: false`、`maximizable: false`、`fullscreen: false`、启动居中，可经原生标题栏拖至任意位置；最小/最大尺寸限制已移除，不再采用有限范围缩放。

### 2. Tauri v2 跨平台版 (Windows / macOS)
- 基于 Rust 语言与系统原生 WebView 构建，安装包小巧克制（~9 MB）。
- `src-tauri/tauri.conf.json` 与 Cocoa 版保持同一窗口规则：`560 × 780`、`resizable: false`、`maximizable: false`、`fullscreen: false`、`center: true`，不再设置 `minWidth` / `maxWidth` / `minHeight` / `maxHeight`。
